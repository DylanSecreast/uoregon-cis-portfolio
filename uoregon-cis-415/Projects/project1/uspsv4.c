...
static void getPidInfo(int pid) {
  //p1putint(1, pid);
  //p1putstr(1, "\n");

  char procIdFp[DEFAULT_PROCS];
  char infoBuf[DEFAULT_PROCS];
  char tempBuf[2048];
  int end;

  sprintf(procIdFp, "/proc/%d/", pid);
  char *sched = "sched";
  strcat(tempBuf, sched);
/*
  while (p1getline(STDIN_FILENO, procIdFp, 2048) != 0) {
    end = p1strchr(procIdFp, '\n');
    tempBuf[end] = '\0';
...
int main(int argc, char *argv[]) {
  ...
  while (p1getline(STDIN_FILENO, buf, BUFFER_SIZE) != 0) {
