...
// Tests for valid card values
bool PlayingCard::isGood(char myValue, char mySuit)
{
	if (myValue == 'A' || myValue == 'T' || myValue == 'J' || myValue == 'Q' || myValue == 'K' || (myValue >= '2' && myValue <= '9'))	// Tests values
	{
		if (mySuit == 'C' || mySuit == 'D' || mySuit == 'S' || mySuit == 'H')	// Tests suits
		{
			return true;
		}
		else
