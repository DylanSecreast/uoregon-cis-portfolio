...
(* Call-by-value parameter passing with dynamic scope
...
fun interp (exp, env) =
  ...
  | AST_APP (exp1, exp2)        => (case (interp (exp1, env)) of
      ...
      | RES_FUN (var, exp) =>
          let val v1 = interp (exp2, env)
          in case v1 of
                RES_ERROR s => v1
              | _ => let val nenv = extend_env(env, var, v1)
...
fun run_str s = interp (parsestr s,new_env ());
