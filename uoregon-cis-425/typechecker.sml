(* Dylan Secreast - CIS 425 - 11/14/16 *)
...
fun typeOf env (Literal x)       = INT
  ...
  | typeOf env (Lett(l,e1,e2))   = raise Unimplemented  (* Disregard Lett: https://piazza.com/class/itkpjrypswb3de?cid=67 *)

  | typeOf env (App(e1,e2))      = (case typeOf env e1 of
                                   ARROW (t1,t2) => if (typeOf env e2 = t1)
                                                    then t2
                                                    else raise TypeError
                                   | _ => raise TypeError)

  | typeOf env (Rec(i,t,e))      = raise Unimplemented  (* Disregard Rec: https://piazza.com/class/itkpjrypswb3de?cid=67 *)
