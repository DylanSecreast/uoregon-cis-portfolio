...
void DoublyLinkedList::insertFirst(long d){
...
void DoublyLinkedList::insertLast(long d){
	Link * newLink = new Link(d);   // make new link
...
bool DoublyLinkedList::insertKey(long key, long d){
...
Link& DoublyLinkedList::deleteKey(long key){
	Link * current = first;          // start at beginning
...
bool DoublyLinkedList::findKey(long key){
	if (first == NULL){
		return false;
	}
