...
Node::Node(int i = 0, double d = 0.0){
	iData = i;
	dData = d;
...
// Tree class declarations
	...
	Node * Tree::find(int key){      // find node with given key                
		if (root == NULL){
	...
	bool Tree::remove(int key){ // remove node with given key                     
		Node * current = root;
		Node * parent = root;
	...
	// Search	for	a	node	containing	the	value	val	and	val	if	such	
	// a	node	exists	in	the	tree.If	val	is	not	present, return	the
	// smallest	value	in	the	tree	that	is	greater	than	val.If	no
	// such	node	exists, return	-1.		(Note, this	means	that	the
	// tree	will	not	work	for	negative	integers.)
	int Tree::findNext(int val){
		if (root == NULL)
			return NULL;    // cant find in empty tree

		Node * current = root;               // start at root
		while (current->iData != val)        // while no match,
		{
			if (val < current->iData)         // go left?
				current = current->leftChild;
			else                            // or go right?
				current = current->rightChild;

			if (current->iData != val){		// if no such node exists
				return -1;					// return -1
			}

			if (current == NULL){					// if no child,
				while (current->leftChild != NULL){
					current = current->leftChild;	// return the smallest value in the tree that is greater than val
				}
	...
	// same as findNext(), but deletes the node if possible. return -1 if no solution found and nothing deleted.
	int Tree::deleteNext(int val){
		if (root == NULL){		// empty tree?
			return NULL;
		}
		...
		if (current->iData == val){
			delete current;		// match found, deleting
		}
